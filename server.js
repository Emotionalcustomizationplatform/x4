...
app.post('/api/recharge', (req, res) => {
  ...
  res.json({ orderId: order.id, paypalLink, isFirst: order.isFirst, bonus: order.bonus });
});
...
app.post('/api/recharge/confirm/:orderId', async (req, res) => {
  ...
  await sendEmail(ADMIN_EMAIL, `💰 充值待确认 - ${order.amount} USD`, `<h2>充值待确认</h2><p>订单: ${order.id}</p><p>用户: ${order.email}</p><p>金额: $${order.amount}</p><p>首充: ${order.isFirst ? '是（赠送 $'+order.bonus+'）' : '否'}</p><p><a href="http://localhost:${port}/api/admin/confirm-recharge/${order.id}?key=${ADMIN_KEY}">确认到账</a></p>`);
  res.json({ success: true });
});

// 管理员确认（实际使用时邮件中的链接需要替换为部署域名）
...
app.post('/api/submit', async (req, res) => {
  ...
  const logEntry = { id: submissionId, name, email, amount: 10, session_type, preferred_time, special_request, ref: referrer, ip: req.ip, ts: new Date() };
  const logFile = path.join(logPath, `leads_${new Date().toISOString().split('T')[0]}.jsonl`);
  fs.appendFileSync(logFile, JSON.stringify(logEntry) + '\n');
  await sendEmail(ADMIN_EMAIL, `🌙 新预约 - ${name}`, `<h2>新预约</h2><p>姓名: ${name}</p><p>邮箱: ${email}</p><p>类型: ${session_type}</p><p>时间: ${preferred_time}</p><p>特殊要求: ${special_request || '无'}</p>`);
...
app.listen(port, '0.0.0.0', () => console.log(`🌙 Luna Whisper 运行在 ${port}`));